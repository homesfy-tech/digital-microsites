...
let apiDataGet = async () => {
    let data = getProjectData(931)       // Change Project_ID Integer Value only Ex:1131
        .then((data) => {
        ...
        .catch((error) => {
            ...
            $(".whatsapp_url").attr("href", whatsapp_url);
            $(".phone_url").attr("href", "tel:" + 918652831118 + "");
            $(".phone_no").html("918652831118")
        });
}
apiDataGet();
// UTM Data Call
...
async function saveLead(name, email, number, country_code, pref) {
    ...
    obj.magnet_id = 419142; //if magnet else blank (int)
    obj.source_id = 14;
