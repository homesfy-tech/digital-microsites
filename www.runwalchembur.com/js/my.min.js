...
let apiDataGet = async () => {
    let data = getProjectData(2655)       // Change Project_ID Integer Value only Ex:1131
        .then((data) => {
...
$('#OnLaodForm').on("submit", function () {
    ...
    saveLead(name, email, number, country_code, 'OnLaod_overview_homepage');
    return !1
})
...
async function saveLead(name, email, number, country_code, pref) {
    ...
    } else if(pref== 'video_overview_homepage'){
        SendLead(obj,"video-tour.html");
    }
     else {
        SendLead(obj, "thankyou.html");
    }
