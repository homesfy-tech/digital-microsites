...
let apiDataGet = async () => {
    getProjectData(3226)       // Change Project_ID Integer Value only Ex:1131
        .then((data) => {
...
async function saveLead(name, email, country_code, number, tracking_lead_id, pref, formId) {
    // Get form-specific selectors based on formId
    const selectors = {
        'dropForm': {
            otpInput: '.dropnOtp',
            fillOtp: '.FillOtp',
            showSection: '.showdrop',
            hideSection: '.hidedrop'
        },
        'mForm': {
            otpInput: '.dropnOtp1',
            fillOtp: '.FillOtp1',
            showSection: '.mfieldshow',
            hideSection: '.mfieldhide'
        },
        'popForm': {
            otpInput: '.dropnOtp2',
            fillOtp: '.FillOtp2',
            showSection: '.popfieldshow',
            hideSection: '.popfieldhide'
        }
    };

    const currentForm = selectors[formId];
    ...
    if ($(currentForm.otpInput).val() == '' && parseInt(number)) {
        // Show OTP section for current form only
        $(currentForm.showSection).show();
        $(currentForm.hideSection).hide();
        ...
        $.post("https://api.homesfy.in/api/users/send_otp", otpData, function(data, status) {
            if (status == "success") {
                $(currentForm.otpInput).val(data.otp);
            }
        });
    }

    // Verify OTP
    if ($(currentForm.otpInput).val() != '' && 
        $(currentForm.otpInput).val() == $(currentForm.fillOtp).val()) {
...
// Example usage in your form submit handlers:
$("#dropForm").submit(function(e) {
    e.preventDefault();
    saveLead(
        $(this).find('[name="name"]').val(),
        $(this).find('[name="email"]').val(),
        $(this).find('[name="country_code"]').val(),
        $(this).find('[name="number"]').val(),
        $(this).find('[name="tracking_lead_id"]').val(),
        'dropForm_pref',
        'dropForm'
    );
});
