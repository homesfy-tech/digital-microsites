...
$("[data-target][data-toggle=modal]").on("click", function () {
  ...
  Title = {
    ...
    Location:
      "Due to huge rush at site we would request you to book your site visit in advance",
    Gallery: "Inside features of the project at a glance",
    video_call: "Virtual Zoom Meetings - With the Experts",
    default:
      "PLEASE SHARE YOUR DETAILS. OUR SALES EXPERT WILL CALL YOU SHORTLY",
    instant: "Instant Call Back",
  };
...
let apiDataGet = async () => {
  let data = getProjectData(3524) // Change Project_ID Integer Value only Ex:1131
    .then((data) => {
      phone_no = data.result.magnet_id
        ? data.result.magnet_number
        : data.result.phone;
      whatsapp_url = data.result.wp_links_sms;
      whatsapp_url = whatsapp_url.split("=");
      whatsapp_url[1] = data.result.magnet_id
        ? "+91" + data.result.magnet_number + "&text"
        : whatsapp_url[1];
    ...
    .catch((error) => {
      var whatsapp_url =
        "https://api.whatsapp.com/send?phone=917304412403&text=Hi!";
      $(".whatsapp_url").attr("href", whatsapp_url);
...
$("#dropEnquiry1").on("submit", function () {
