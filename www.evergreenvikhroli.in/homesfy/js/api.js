...
let getProjectData = async (project_id) => {
  ...
  if (project_id && magnet_id) {
    const api_url =
      "https://api.homesfy.in/api/leads/projectdata/" +
      project_id +
      "?magnet_id=" +
      magnet_id;
    ...
    const api_url =
      "https://api.homesfy.in/api/leads/projectdata/" + project_id;
    return await $.get(api_url);
  } else {
...
var queryForm = (settings = null) => {
  ...
  if (querystring.length > 1) {
    ...
    for (i in pairs) {
      ...
      if (keyval[0] == "utm_source") resultJson.utmsource = keyval[1];
      // console.log(resultJson.utmsource)
      if (keyval[0] == "utm_medium") resultJson.utmmedium = keyval[1];
      // console.log(resultJson.utmmedium)
      if (keyval[0] == "utm_campaign") resultJson.utmcampaign = keyval[1];
      // console.log(resultJson.utmcampaign)
      if (keyval[0] == "utm_content") resultJson.utmcontent = keyval[1];
      if (keyval[0] == "utm_term") resultJson.utmterm = keyval[1];
...
//   var getAddress = await fetch("https://jsonip.com", { mode: "cors" }).then(
//     (resp) => resp.json()
//   );
//   var ipAddress = null;
